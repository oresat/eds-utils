...
def write_rst(eds: EDS, file_path='', dcf=False) -> None:
    ...
    line = '   "Modification Time", "' + eds.file_info.modification_dt.strftime('%I:%M%p') + '"'
    lines.append(line)
    line = '   "Modification Date", "' + eds.file_info.modification_dt.strftime('%m-%d-%Y') + '"'
    lines.append(line)
    ...
    lines.append('.. csv-table::')
    lines.append('')
    lines.append(f'   "Vendor Name", "{eds.device_info.vender_name}"')
    lines.append(f'   "Vendor Number", "{eds.device_info.vender_number}"')
    ...
    for i in BAUD_RATE:
        lines.append(f'   "Baud Rate {i}", "{int(eds.device_info.baud_rate[i])}"')
    ...
    line = f'   "Dynamic Channels Supported", "{int(eds.device_info.dynamic_channel_supperted)}"'
    lines.append(line)
    ...
    if dcf:
        ...
        lines.append('.. csv-table::')
        lines.append('')
        lines.append(f'   "Node ID", "0x{eds.device_commissioning.node_id:X}"')
        lines.append(f'   "Node Name", "{eds.device_commissioning.node_name}"')
        lines.append(f'   "Baudrate", "{eds.device_commissioning.baud_rate}"')
        lines.append(f'   "Net Number", "{eds.device_commissioning.net_number}"')
        lines.append(f'   "Network Name", "{eds.device_commissioning.network_name}"')
        lines.append(f'   "CANopen Manager", "{int(eds.device_commissioning.canopen_manager)}"')
        lines.append(f'   "LSS SerialNumber", "{eds.device_commissioning.lss_serialnumber}"')
        lines.append('')
